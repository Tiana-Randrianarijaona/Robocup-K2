...
class Tracker():
	def __init__(self,
			...
			#for
			yellowLower = (165,77,76),
			yellowUpper = (179,183,146)	):
	...
	def callBack(self, frame_from_image_topic):
		...
		if len(cnts) > 0:
			...
			center = (int(center_rect[0]), int(center_rect[1]))
			...
			if w > 200:
				...
				distance_goal_center_camera = self.trackerHelper.getCoordinates(center_rect[0],center_x)
		...
		return {"frame": frame, "width":w, "x":distance_goal_center_camera}		
